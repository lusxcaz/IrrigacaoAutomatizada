...
const char* ssid = "Toca_da_Jade";             //SSID da rede WIFI
const char* password =  "analista";    //senha da rede wifi
//Informações da Instância do broker MQTT
const char* mqttServer = "broker.mqtt-dashboard.com";   //server
const char* mqttUser = "lucas";              //user
const char* mqttPassword = "251201";      //password
...
void loop() {
  ...
  //Definindo o parâmetro para um Solo úmido, acendendo o LED VERDE
    if (valor_analogico >= 0 && valor_analogico < 500)
    {
      ...
      sprintf(mensagem, "0"); //Definindo o valor zero como parâmetro para o Broker mostrar uma imagem "verde"
  ...
  //Definindo o parâmetro para um Solo Seco, acendendo o LED VERMELHO e acionando a bomba
    if (valor_analogico >= 500 && valor_analogico <= 1024)
    {
      ...
      digitalWrite (pino_bomba, HIGH);
       ...
       //Enviando Mensagem ao Broker
      sprintf(mensagem, "1"); //Definindo o valor UM como parâmetro para o Broker mostrar uma imagem "vermelha"
      Serial.print("Mensagem enviada: ");
      Serial.println(mensagem); 
      client.publish("jardim/bomba", mensagem);
      Serial.println("Mensagem enviada com sucesso...");
